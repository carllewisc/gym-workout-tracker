...
import { Play, Pause, CircleStopIcon as Stop, Plus, X, Dumbbell, Clock, ChevronUp, ChevronDown } from 'lucide-react'
...
const WorkoutTracker = () => {
  ...
  const handleAddExercise = () => {
    if (selectedExercise) {
      setExercises(prev => [...prev, { id: Date.now().toString(), name: selectedExercise, sets: [{ weight: 0, reps: 0 }] }])
  ...
  const handleAddSet = (exerciseId: string) => {
    setExercises(prev => prev.map(ex =>
      ex.id === exerciseId
        ? { ...ex, sets: [...ex.sets, { weight: ex.sets[ex.sets.length - 1].weight, reps: ex.sets[ex.sets.length - 1].reps }] }
        : ex
    ))
  }
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-2xl font-bold flex justify-between items-center">
            ...
            {isOffline && (
              <span className="text-sm font-normal text-yellow-500">Offline Mode</span>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex justify-between items-center mb-4">
            <div className="space-x-2">
              {workoutState === 'notStarted' && (
                <Button onClick={handleStartWorkout}><Play className="mr-2 h-4 w-4" /> Start</Button>
              )}
              {workoutState === 'inProgress' && (
                <>
                  <Button onClick={handlePauseWorkout}><Pause className="mr-2 h-4 w-4" /> Pause</Button>
                  <Button onClick={handleEndWorkout} variant="destructive"><Stop className="mr-2 h-4 w-4" /> End</Button>
                </>
              )}
              {workoutState === 'paused' && (
                <>
                  <Button onClick={handleStartWorkout}><Play className="mr-2 h-4 w-4" /> Resume</Button>
                  <Button onClick={handleEndWorkout} variant="destructive"><Stop className="mr-2 h-4 w-4" /> End</Button>
                </>
              )}
            </div>
        ...
        <CardContent>
          <div className="flex space-x-2 mb-4">
            ...
            >
              ...
              <SelectContent>
                {selectedMuscleGroup && exerciseData[selectedMuscleGroup].map(exercise => (
