...
const ExerciseSelection: React.FC<ExerciseSelectionProps> = ({ onSelectExercise }) => {
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      ...
      try {
        ...
        setRecentExercises([exerciseData.Chest[0], exerciseData.Back[0], exerciseData.Legs[0]]);
        setFavoriteExercises([exerciseData.Chest[1], exerciseData.Back[1], exerciseData.Legs[1]]);
        setIsLoading(false);
      } catch (err) {
  ...
  const filteredExercises = Object.entries(exerciseData).reduce(
    (acc, [group, exercises]) => {
        ...
        <Tabs defaultValue="quick-access" className="w-full">
          ...
          <TabsContent value="quick-access">
            <div className="space-y-4">
              <div>
                <h3 className="mb-2 text-lg font-semibold">Recent Exercises</h3>
                <ScrollArea className="h-[100px] w-full whitespace-nowrap rounded-md border">
                  <div className="flex space-x-4 p-4">
                    {isLoading ? (
                      Array(3)
                        .fill(0)
                        .map((_, i) => <Skeleton key={i} className="h-[32px] w-[100px] rounded-full" />)
              ...
              <div>
                <h3 className="mb-2 text-lg font-semibold">Favorite Exercises</h3>
                <ScrollArea className="h-[100px] w-full whitespace-nowrap rounded-md border">
                  <div className="flex space-x-4 p-4">
                    {isLoading ? (
                      Array(3)
                        ...
                        {exercises.map((exercise) => (
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Button variant="ghost" size="icon" onClick={() => toggleFavorite(exercise)}>
                                <Heart
                                  className={`h-4 w-4 ${
